...
def pro(img,mask,draw_rect1=True,draw_rect2=True,draw_lines=True,draw_mask=True):
    ...
    for face in faces:
        ...
        (b11, jacobian) = cv2.projectPoints(np.array([(350.0, 350.0, 100.0)]), rotation_vector, translation_vector, camera_matrix, dist_coeffs)
        (b12, jacobian) = cv2.projectPoints(np.array([(-350.0, -350.0, 100.0)]), rotation_vector, translation_vector, camera_matrix, dist_coeffs)
        (b13, jacobian) = cv2.projectPoints(np.array([(-350.0, 350, 100.0)]), rotation_vector, translation_vector, camera_matrix, dist_coeffs)
        (b14, jacobian) = cv2.projectPoints(np.array([(350.0, -350.0, 100.0)]), rotation_vector, translation_vector, camera_matrix, dist_coeffs)
